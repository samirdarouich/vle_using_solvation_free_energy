...
def get_lf_training_data(lf_databank: str, lf_mixture: str, lf_unique_key: str ) -> Tuple[List[List], List[List], List[List]]:
    """
    ...
    Args:
        lf_databank (str): path to databank
    ...
    for ni,component in enumerate( lf_mixture.split("_") ):
        
        # If component is not in lf dict, then skip it
        if not component in lf_dict[lf_mixture][lf_unique_key].keys(): continue

        l_learn[ni]     = [ np.array(item[1]["lambda"]) for item in lf_dict[lf_mixture][lf_unique_key][component].items() ] 
        xi_learn[ni]    = [ np.ones(len(lf_dict[lf_mixture][lf_unique_key][component][xi]["lambda"]))*float(xi) for i,xi in enumerate(lf_dict[lf_mixture][lf_unique_key][component].keys()) ] 
        du_dl_learn[ni] = [ np.array(item[1]["du_dl"]) for item in lf_dict[lf_mixture][lf_unique_key][component].items() ] 
