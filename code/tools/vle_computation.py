...
def compute_equilibrium_temperatures(x1,temperatures,y1,y1_std,settings,x_ref=[None],y_ref=[None],t_ref=[None]):
    ...
    labels = [  "", "", "Reference", "", "$x_\mathrm{%s}$ / -"%settings["components"][0], "$T$ / K" ]
    colors = [ "tab:blue", "tab:blue",  "black", "black" ]
    ls     = [ "solid", "solid", "None", "None"]
    marker = [ ".", ".", "*", "*" ]

    save_path = settings["T_save_path"]

    plot_data(data,labels,colors,save_path,ax_lim=[[0.0,1.0]],linestyle=ls,markerstyle=marker)

    return

def compute_equilibrium_pressure(x1,y1,temperatures,dG_mix,dG_std_mix,density_mix,settings,x_ref=[None],y_ref=[None],p_ref=[None]):
    """
    ...
    if any(p_ref):
        ...
        labels = [ "", "MRD: %.2f %%"%p_err, "", "Reference" , "$x_\mathrm{%s}$, $y_\mathrm{%s}$ / -"%(settings["components"][0],settings["components"][0]), "$p$ / bar" ]
    else:
        labels = [ "", "", "$x_\mathrm{%s}$, $y_\mathrm{%s}$ / -"%(settings["components"][0],settings["components"][0]), "$p$ / bar" ]


    data   = [ [x1,p_equib,None,p_equib_std], [y1,p_equib,None,p_equib_std], [x_ref,p_ref,None,None], [y_ref,p_ref,None,None] ]

    colors = [ "tab:blue", "tab:blue", "black", "black" ]
    ls     = [ "solid", "solid", "None", "None"]
    marker = [ "None", "None", "*", "*" ]

    save_path = settings["p_save_path"]

    plot_data(data,labels,colors,save_path,ax_lim=[[0.0,1.0]],linestyle=ls,markerstyle=marker)

    return p_equib, p_equib_std

def compute_vapor_mole_fraction(x1,temperatures,dG_mix,dG_std_mix,settings,x_ref=[None],y_ref=[None]):

    """
    Function that computes the vapor mole fraction (and standard deviation) based on free solvation energy results.
    ...
    if any(y_ref):
        ...
        labels = [ "", "MRD: %.2f %%"%y_err, "Reference" , "$x_\mathrm{%s}$ / -"%settings["components"][0], "$y_\mathrm{%s}$ / -"%settings["components"][0] ]
    else:
        labels = [ "", "", "", "$x_\mathrm{%s}$ / -"%settings["components"][0], "$y_\mathrm{%s}$ / -"%settings["components"][0] ]
    ...
    save_path = settings["y1_save_path"]
...
def compute_gamma(x1,temperature,dG_mix,dG_pure,dG_std_mix,dG_std_pure,
                  density_mix,density_pure,settings,
                  x_ref=[None],gamma1_ref=[None],gamma2_ref=[None]):
    ...
    Args:
        ...
        densitiy_mix (1D array): Mass densitiy in each composition.
        densitiy_pure (1D array): Mass densitiy for each pure component.
    ...
    gamma = [ np.exp( (dG_mix[i]-dG_pure[i]) / (8.314*temperature) ) * density_mix / density_pure[i]  for i in range(2)]

    gamma_std = [ gamma[i] * 1/(8.314*temperature) * np.sqrt( dG_std_mix[i]**2 + dG_std_pure[i]**2 ) for i in range(2)]
    ...
    labels = [ *settings["components"], "", "Reference", "$x_\mathrm{%s}$ / -"%settings["components"][0], "$\gamma$ / -" ]
    colors = [ "tab:blue", "tab:orange", "black", "black" ]
    ls     = [ "solid", "solid", "None", "None"]
    marker = [ ".", ".", "*", "*" ]

    save_path = settings["gamma_save_path"]
